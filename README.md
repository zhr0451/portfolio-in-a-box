...
Небольшой статический сайт-портфолио на Astro 6 и Tailwind CSS 4. Сейчас проект собран как одна страница с данными из одного файла и деплоится на GitHub Pages.
...
- Локализация `ru/en` переключается на клиенте через inline-скрипт
...
- `src/content.config.ts` пока пустой: content collections еще не используются
...
- Оставить `src/pages/` только для маршрутов, а UI держать в `src/components/`
